  ...
  const arrFilterByParam = (arr, paramS) => {
    return new Promise((resolve) => {
      let find = arr.map((element) => {
        if (element[paramS] !== undefined &&element[paramS] !== false &element[paramS] !== "false" ) {
          return element[paramS];
        }else{
          return false
        }
      });
      let result = find.filter(
  ...
  const findTxt = async (word) => {
    ...
    if (word.length > 0) {
      ...
      if (resultSearch.length === 0 && data.paramSearch2) {
        let resultSearch2 = await handleChangeValueInput(word, arrs.arr2);
